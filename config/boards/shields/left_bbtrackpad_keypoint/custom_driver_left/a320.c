...
/* ========= Mode key listener =========
 *
 * Key positions 48 and 49 are the two inner thumb keys. Both are layer-taps, so
 * holding one sends nothing to the host -- which is the whole reason the mode
 * key lives on a layer-tap and not on a mod-tap. */
...
static void a320_work_cb(struct k_work *work) {
    ...
    if (scroll_mode_active) {
        ...
        const float fx = motion_cursor_scale(&a320_cursor, dx, speed_preference, 1.0f);
        const float fy = motion_cursor_scale(&a320_cursor, dy, speed_preference, 1.0f);
